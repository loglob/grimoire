...
namespace Index
{
	...
	*/
	...
	function urlParams() : string
	{
		return selectedSources()
			.map(x => `from=${encodeURIComponent(x)}`)
			.concat(`q=${encodeURIComponent(Table.searchField.value)}`)
			.join('&');
	}
	...
	export function initUI()
	{
		...
		document.getElementById("spell-card-view")
			.onclick = _ => window.location.href = `cards.html?${urlParams()}`;
	}
